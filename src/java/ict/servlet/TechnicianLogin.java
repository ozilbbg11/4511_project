 ...
 * @author raymo
 */
@WebServlet(name = "TechnicianLogin", urlPatterns = {"/technicianLogin"})
public class TechnicianLogin extends HttpServlet{
   ...
   private TechnicianDB db;
    ...
    private void doAuthenticate(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String targetURL;
        TechnicianBean bean = db.isValidUser(id, pwd,"technician");
        if (bean.getName() != null) {
            HttpSession session = request.getSession(true);
            session.setAttribute("TechnicianBean", bean);
            targetURL = "/inventory.jsp";
            
        } else {
            targetURL = "/loginError.jsp?format=tech";
        }
    ...
    private void doLogin(HttpServletRequest request, HttpServletResponse response)
        ...
        String targetURL = "tech_login.jsp";
        RequestDispatcher rd;
        rd = getServletContext().getRequestDispatcher("/" + targetURL);
        rd.forward(request, response);
    }
